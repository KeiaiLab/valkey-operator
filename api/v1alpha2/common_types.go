...
type AuthSpec struct {
	...
	// operator 가 *직접 회전 수행* — 자체 재설계(ESO 위임 대체). RotationPolicy(외부
	// 회전 *반영*만)와 책임 분리.
	// +optional
...
type CertManagerSpec struct {
	...
	// 외부 chart 의 `tls.autoGenerated` 패턴과 동등. IssuerRef.Name 과 상호
	// 배타 (webhook reject).
	// +optional
...
// shard 별 PDB N건 (selector = valkey.keiailab.io/shard) 생성. v1alpha1 동일 필드의 hub 동기
// (JSON byte-copy conversion 정합 — tag 동일 = 자동 매핑, round-trip 손실 0).
