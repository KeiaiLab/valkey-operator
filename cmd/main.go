...
func main() {
	...
	// cycle 74: WATCH_NAMESPACES env (chart values.watch.namespaces) — comma-separated.
	// 미설정 시 cluster-wide watch (default). 설정 시 cache.DefaultNamespaces 으로
	// 제한 — multi-tenant 또는 RBAC namespaced Role 환경 지원.
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "ef6cc5c8.keiailab.io",
		Cache:                  cacheOptions,
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1alpha1.SetupValkeyClusterWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "Failed to create webhook", "webhook", "ValkeyCluster")
			os.Exit(1)
		}
	}
	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1alpha1.SetupValkeyWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "Failed to create webhook", "webhook", "Valkey")
			os.Exit(1)
		}
	}
