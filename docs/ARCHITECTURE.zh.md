...
- **稳定度等级**: v1.0.13 (standalone + replication + cluster 已 GA; federation 处于 alpha)
...
依据 `operator-commons/ARCHITECTURE.md` 的矩阵,采用率为 **8/8 (100%)** — *carbon-copy 参考实现*。

| 包 | 状态 | 用途 |
|---|---|---|
| `pkg/security` | ✅ | restricted PSA SecurityContext (it8) |
| `pkg/version` | ✅ | Valkey 版本 allowlist (it8) |
| `pkg/labels` | ✅ | 推荐 label (it29) |
| `pkg/monitoring` | ✅ | ServiceMonitor reconciler (it23) |
| `pkg/networkpolicy` | ✅ | Deny-by-default + 可选项 (it25) |
| `pkg/webhook` | ✅ | Validation 辅助函数 (it31) |
| `pkg/finalizer` | ✅ | `Add` / `Remove` / `Has` |
| `pkg/status` | ✅ | Condition reason |

valkey 是 *首个 100% 采用者* — mongodb / postgres 在各自迁移时把本仓库作为参考。
