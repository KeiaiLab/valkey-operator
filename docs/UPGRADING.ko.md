...
```bash
# go.mod 의 operator-commons 의존 bump 이후
go mod tidy
```

- **신규 import**: `github.com/keiailab/operator-commons/pkg/pvc`,
  `pkg/topology`
...
마이그레이션 영향:
- Reconcile 동작은 동일 (refactor 만, 외부 동작 변경 없음)
- CRD spec 변경 없음
- Helm chart 영향 없음
...
상세는 operator-commons ADR-0013 (audit-production-grade.sh) 에서 확인할 수
있다.
