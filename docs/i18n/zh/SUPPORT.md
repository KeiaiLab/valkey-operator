...
| 您遇到 Prometheus 告警,需要 MTTR (Mean Time To Recovery) 步骤。 | [`docs/operations/runbook.md`](../../operations/runbook.md) §9 (每条告警的 `runbook_url` 注解都会指向此处)。 |
| 您看到异常行为但没有告警。 | [`docs/operations/troubleshooting.md`](../../operations/troubleshooting.md) — 症状 → 原因 → 诊断 → 处置流程图。 |
...
2. 尝试 [故障排查流程图](../../operations/troubleshooting.md)。
3. 在报告中准备好以下信息:
