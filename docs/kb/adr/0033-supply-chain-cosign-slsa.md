...
| 차트 | 공급망 보증 |
|---|---|
| Bitnami redis v25.5.2 | in-toto attestation + Photon Linux CVE scanning |
| Cloudpirates redis v0.27.6 | cosign signed |
...
- 외부 Bitnami / Cloudpirates 와 동등 보증 수준 — 사용자 이탈 차단.
...
- *cosign* (Sigstore 표준) vs *Notary v2* / *PGP signing* — cosign 이
  Bitnami in-toto + Cloudpirates 와 호환 + Sigstore 생태계 가장 활발.
...
- 외부 패턴: Bitnami in-toto attestation, Cloudpirates cosign signed.
