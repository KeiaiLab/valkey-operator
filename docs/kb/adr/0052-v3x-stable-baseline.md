...
- C (커뮤니티): ✅ ADOPTERS.md / CONTRIBUTING / CODE_OF_CONDUCT / SECURITY / GOVERNANCE / CloudPirates 호환성 매트릭스 (valkey-ADR/0043) / SLSA Level 3 + cosign supply chain (valkey-ADR/0046) / ArtifactHub 서명 배지 (valkey-ADR/0044)
...
- **상업 호환**: valkey-ADR/0042 (commercial parity series closure) + valkey-ADR/0043 (CloudPirates 호환 매트릭스).
...
- **외부 신뢰** — audit 자동 측정 (commons-ADR/0013) + 본 baseline ADR + 거버넌스 4종 + 공급망 보안 (SLSA L3 + cosign) + CloudPirates 호환 매트릭스의 5 축이 *상용 등급* 신뢰 신호로 작용.
...
- **상업 대안** — CloudPirates / Bitnami 비교 매트릭스로 외부 사용자 평가 자료 자체 제공.
...
- valkey-ADR/0043: CloudPirates valkey 호환 매트릭스
