# Valkey StatefulSet → ValkeyCluster 마이그레이션 (한국어)
...
| 문서 | 시나리오 |
|---|---|
| [zero-downtime.ko.md](zero-downtime.ko.md) | 일반 마이그레이션 (5단계, < 30분, 0초 다운타임) |
| [secondary-promote.ko.md](secondary-promote.ko.md) | Replica → Primary promote 후 기존 primary 폐기 (데이터 정합성 우선) |
| [rollback.ko.md](rollback.ko.md) | 마이그레이션 중 client 이상 발견 시 즉시 원복 |

## 사전 점검 체크리스트 (Pre-flight checklist)
