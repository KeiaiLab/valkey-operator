# ValkeyCluster Per-Shard PDB (CDEX-M2)
...
`PerShard=true` (CDEX-M2 opt-in, MR `!8`) 로 활성화 시 reconciler 는 *shard 별 PDB N건* 을 생성한다:
...
- CDEX-M2 implementation: MR `!8` (merge_commit `e7400633`) — `internal/resources/pdb.go` `BuildShardPDB`
- CDEX-M2 CRD regen: MR `!9` (merge_commit `fdc12941`) — PerShard field schema
- CDEX-M1 sister (PDB delete path): MR `!7` (merge_commit `c5d289c`) — `EnsurePDBDeleted` helper
- mongodb sharded pattern: `mongodb-operator/internal/resources/builder.go:2105`
- Unit tests: `internal/controller/pdb_default_test.go` (BuildShardPDB + EnsurePDBDeleted 11 case PASS)
