...
func (r *ValkeyClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrlResult ctrl.Result, retErr error) {
	...
	if !commonsfinalizer.Has(vc, finalizerValkeyCluster) {
		commonsfinalizer.Add(vc, finalizerValkeyCluster)
	...
	authConf := resources.BuildAuthConfSecret(vc.Name, vc.Namespace, "valkey-cluster", password, false, "")
	if err := applyAuthConfSecret(ctx, r.Client, r.Scheme, vc, authConf); err != nil {
		return applyErrorCondition(ctx, r.Client, vc, "AuthConfSecret", err, r.Recorder)
	}
	authConfSecretName := ""
	if authConf != nil {
	...
	// TLS Issuer (cert-manager) — AutoSelfSigned=true 시 namespace-scope SelfSigned
	// Issuer 자동 생성 (외부 chart 의 tls.autoGenerated 패턴 동등). Certificate
	// 보다 먼저 apply.
